...
func assertExpectedObject(
	...
	case bool:
		err := assertBooleanObject(expected, actual)
		if err != nil {
			t.Errorf("testBooleanObject failed: %s", err)
		}
	case string:
		err := assertStringObject(expected, actual)
		if err != nil {
	...
	case []int:
		...
		for i, expectedElem := range expected {
			err := assertIntegerObject(int64(expectedElem), array.Elements[i])
			if err != nil {
				t.Errorf("testIntegerObject failed: %s", err)
			}
		}
	case map[object.HashKey]int64:
		...
		for expectedKey, expectedValue := range expected {
			...
			err := assertIntegerObject(expectedValue, pair.Value)
			if err != nil {
				t.Errorf("testIntegerObject failed: %s", err)
			}
		}
...
func assertBooleanObject(expected bool, actual object.Object) error {
